...
package com.exactpro.th2.ldsprovider.entities.filters
...
class PredicateFactory<T>(
    private val filters: Map<FilterInfo, (FilterRequest) -> Filter<T>>
) {
    ...
    private var containedFiltersInit: Map<String, (FilterRequest) -> Filter<T>>
    ...
    fun build(requestMap: Map<String, List<String>>): FilterPredicate<T> {
        val filtersList = mutableListOf<Filter<T>>().apply {
            for (filterName in requestMap["filters"] ?: emptyList()) {
                ...
                add(constructor(HttpFilter(filterName, requestMap)))
            }
        }
        return FilterPredicate(filtersList)
    }

    fun build(filters: List<com.exactpro.th2.dataprovider.grpc.Filter>): FilterPredicate<T> {
        val filtersList = mutableListOf<Filter<T>>().apply {
            for (filter in filters) {
                ...
                add(constructor(GrpcFilter(filter.name.filterName, filter)))
            }
        }
        return FilterPredicate(filtersList)
    }
